...
def point_align(cfg, input_xyz_points, num_points_per_scene, depth_map, hand_center_3d, cam_intr):
    input_points = input_xyz_points.clone()
    input_points = input_points.reshape((-1, num_points_per_scene, 3))
    batch_size = input_points.shape[0]
    xyz = input_points * 2 / cfg.recon_scale + hand_center_3d.unsqueeze(1)
    ...
    align_z = torch.nn.functional.grid_sample(depth_map, uv_2d, align_corners=True)[:, :, :, 0].transpose(1, 2)

    uv_2d = uv_2d.squeeze(2).reshape((-1, 2))
    align_z = align_z.reshape((uv_2d.shape[0], -1))

    align_xyz = torch.cat((input_xyz_points.reshape(batch_size * num_points_per_scene, 3)[:, :2], align_z), dim=-1)
